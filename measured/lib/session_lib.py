...
def _session_cache_key() -> str:
    """A per-caller identity that's stable for one Claude session but cheap to
    compute.

    `session_dir()` is deterministic given the Claude ancestor, but finding it
    means walking the process tree (several `ps`/proc reads). The caller's own
    parent — the process that re-invokes us each time, e.g. a hook fired per
    tool call — has a fixed PID for the session, so we key on that. PIDs get
    recycled, so we pair the PID with its start time: a recycled PID has a
    different start time and simply misses the cache rather than reusing a
    stale path.
    """
    ppid = os.getppid()
    return f"{ppid}-{parent_start_time(ppid)}"


def cached_session_dir() -> pathlib.Path:
    """`session_dir()` with an on-disk cache, for callers invoked repeatedly
    within a session (notably the PreToolUse hook).

    The hook is a fresh process per tool call, so an in-memory cache buys
    nothing — the result is memoised to a file under `state_root()` keyed by
    `_session_cache_key()`. A hit skips the full ancestor walk; a miss computes
    `session_dir()` once and writes it. Any cache read/write failure falls back
    to computing directly, so the cache can only speed things up, never break
    them.
    """
    try:
    ...
    cache_file = state_root() / "cache" / "session-dir" / key
    try:
        cached = pathlib.Path(cache_file.read_text().strip())
        if cached.is_dir():
            return cached
    except OSError:
        pass
