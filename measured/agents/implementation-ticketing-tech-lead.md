...
You are given a **plan dir path**; it is the directory holding the architecture plan and the tickets you create. Build each file's path by joining its filename to that dir, written below as `<plan-dir>`.

1. Read the architecture plan at `<plan-dir>/ARCHITECTURE.md`. It is the source of truth for what gets built; your tickets decompose it. Every ticket should trace back to a part of the plan, and the tickets together should deliver it with no gaps.
...
6. Draft and revise the task series. Each task lives in its own file at
   `<plan-dir>/TASK-N.md`, numbered from 1 in dependency order. Use the
   standard `Write`, `Read`, and `Edit` tools to create each file. Do not reuse
   one filename for multiple tasks. List the dir's `TASK-N.md` files to see the
   tasks you've created so far.
7. Self review:
...
Each task is its own `TASK-N.md` file at `<plan-dir>/TASK-N.md`. Number them in the order they should be done — their numbers run in dependency order, starting at 1.

Every task file uses exactly these fields. Title in imperative mood. Omit a field only when it doesn't apply (say so rather than leaving it blank).

---

# Task N: Imperative, specific title

Use the file's number (e.g. `Task 42`) so the title matches its filename.

Action-oriented and specific — "Add rate limiting to /api/auth endpoints", not "Rate limiting".

## Context / Background

One to three sentences: why this task exists and what problem it solves. Reference the section of the architecture plan this task implements (it lives at `<plan-dir>/ARCHITECTURE.md`). The implementer shouldn't need to go digging.
