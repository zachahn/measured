...
Review the ticket. `Read` it from the path printed by `measured-session-dir --ticket`. We want to create an excellent ticket that provides enough context for an engineer to implement this task with no blockers.
