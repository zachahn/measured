...
Both phases write into one **plan** — a directory holding the architecture plan and its tickets, persisted across sessions. Allocate it once, up front, and pass its reference to every teammate:
...
1. Spawn a teammate using the subagent: `measured:implementation-planning-architect`. Tell it the plan reference.
...
4. Spawn a teammate using the subagent: `measured:implementation-ticketing-tech-lead-for-agents`. Tell it the plan reference and that the architecture plan is done; it reads the plan and breaks it into tickets that reference it.
