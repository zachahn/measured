...
These keys govern how Claude commits in this repo. The **measured-behavior** plugin owns them: its `commit-settings` hook states the ones you set at the start of every turn, so they apply to all work, not just the implement skill. Setting none of them leaves Claude's default behavior alone.
...
| `commit-settings-for-agents` | Whether spawned subagents get these settings appended to their prompt. `true` or `false`. Off unless set to `true`. |
