...
7. Aggregate, de-duplicate, and sort issues by priority. Verify that all steps were taken. Include the summary of changes.
8. Use `ExitWorktree(discard_changes: true)`. Prefer removing the directory as we have made no changes. Avoid confirming; it is easy to rebuild this worktree.
